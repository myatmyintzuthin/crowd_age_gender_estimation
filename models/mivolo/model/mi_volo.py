...
import numpy as np
import torch
from models.mivolo.data.misc import prepare_classification_images
from models.mivolo.model.create_timm_model import create_model
from models.mivolo.structures import PersonAndFaceCrops, PersonAndFaceResult
...
class MiVOLO:
    ...
    def inference(self, model_input: torch.tensor) -> torch.tensor:
        ...
        with torch.no_grad():
            ...
            end_time = time.time()
            _logger.info(f"Mivolo Speed: '{end_time-start_time}' sec")
    ...
    def predict(self, image: np.ndarray, detected_bboxes: PersonAndFaceResult):
        ...
        detected_bboxes = self.fill_in_results(output, detected_bboxes, faces_inds, bodies_inds)
